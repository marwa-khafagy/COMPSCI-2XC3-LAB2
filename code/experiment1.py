...
def proportionality_test(const_node_count, max_edge_count, trial_count):
    ...
    for e in range(max_edge_count):
        ...
        print(
            f"Plotted Point ({e},{cyclicalAverageCount}). ({e}/{max_edge_count})")
...
def edge_additions_until_cycle_test(node_counts, trials_per_node_count):
    ...
    minEdgeCountPerNodeCount = PlotGroup(
        "Minimum Edge Additions until Cycle", "#b4daff")
    maxEdgeCountPerNodeCount = PlotGroup(
        "Maximum Edge Additions until Cycle", "#d4b4ff")
    ...
    for i in node_counts:
        ...
        for _ in range(trials_per_node_count):
            ...
            while countUntilCycle < maximumPossibleEdgeCount:
                ...
                if has_cycle(testGraph):
                    ...
                    # Good to Know
                    minEdgesAddedForCycle = smin(
                        minEdgesAddedForCycle, countUntilCycle)
                    maxEdgesAddedForCycle = smax(
                        maxEdgesAddedForCycle, countUntilCycle)
        ...
        print(
            f"Plotted Point ({i},{edgeCountSum / trials_per_node_count}), m:{minEdgesAddedForCycle}, M:{maxEdgesAddedForCycle}.")
...
if (__name__ == "__main__"):
    ...
    # Low Rest
    #test5 = edge_additions_until_cycle_test(range(5), 1000)

    # Good Basis
    #test6 = edge_additions_until_cycle_test(range(100), 1000)
