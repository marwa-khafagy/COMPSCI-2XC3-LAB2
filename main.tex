...
Imagine a graph with every possible edge added to it. We have already stated that the number of edges
\begin{center}
    $E = \frac{V^2 + V}{2}$
\end{center}

Now we think how to force the graph to be un connected. To do this, we pick some node and remove every connected to it. Since, in our graph, every
node is connected to the other $V-1$ nodes, we can remove those $V-1$ connections.

This leaves us with our final graph, where every node is connected to every other node, save for one extra that is only connected to itself (which does not impact connectivity).

To force the graph to be connected, we just add one more edge to the isolated node, which must then, by our definition, connect the entire graph.
